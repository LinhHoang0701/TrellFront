...
const NotificationPopper = ({
  ...
  function giveCombinedNotifications(user, chats) {
    ...
    if (user?.groupNotifications?.length > 0) {
      user.groupNotifications?.forEach((notification) =>
        output.push(notification)
      );
    }
    filteredChats = chats?.map((chat) =>
      chat?.unreadMessages?.filter(
        (message) => message?.sender?._id !== user?._id
      )
    );
    output = output
      .concat(filteredChats)
      .flat()
      .sort(
        (a, b) =>
  ...
  async function handleNotification(notification, chats) {
    ...
    if (notification?.chat?.isGroupChat) {
      notificationToDelete = chats?.filter(
        (chat) => chat?._id === notification?.chat?._id
      );
    } else {
      notificationToDelete = chats?.filter(
        (chat) => chat?._id === notification?.chat
      );
    }
    dispatch(toggleChat(notificationToDelete[0]));
    await dispatch(
      removeUnread({ chat: notificationToDelete[0], userId: user?._id })
    );
    if (notification?.chat?.isGroupChat) {
  ...
  return (
    ...
    <>
      <div
        className="hidden w-96 md:!flex flex-col shadow-sm drop-shadow-md transition-all bg-white absolute z-[50] top-20 left-8 rounded-md overflow-y-auto scrollbar-thin scrollbar-thumb-purple-200 scrollbar-track-purple-50 max-h-[50%] "
      ...
      >
        <div className="flex items-center  border-b-[1.5px] border-gray-100 px-4 py-3 mb-2  drop-shadow-sm">
          <h2 className="font-nunito text-lg font-semibold text-gray-700 flex-1">
            Notifications
          </h2>
        </div>
        {user?.groupNotifications.length > 0 ||
        chats?.some((chat) => checkMessage(chat)) ? (
          giveCombinedNotifications(user, chats)?.map((notification, index) => (
            <>
              <div
                key={index}
                className="w-[22.5rem] mx-auto flex items-center rounded-md px-3 py-3 my-1 hover:bg-gray-100 cursor-pointer transition-all hover:drop-shadow-sm "
                onClick={() => handleNotification(notification, chats)}
              >
                ...
                {notification?.chat?.isGroupChat ? (
                  <p className="font-nunito text-sm w-full max-w-[75%] ">
                    New message recieved in{" "}
                    <span className="capitalize font-bold text-purple-800">
                      {notification?.chat?.groupName}
                    </span>{" "}
                    {getCorrectTime(notification?.updatedAt)}
                  </p>
                ) : (
                  <p className="font-nunito text-sm w-full max-w-[75%]">
                    <span className="capitalize font-bold text-purple-800">
                ...
                {notification?.chat?.isGroupChat ? (
                  <div className="w-8 h-8 rounded-full ml-5">
                    <Avatar
                      src={notification?.chat?.groupImage}
                      alt="avatar"
          ...
          <div className=" my-3 px-4 py-3 bg-purple-200 w-[93%] mx-auto rounded-md">
            <p className="font-nunito text-center text-sm  ">
      ...
      {/* for mobile screen  */}
      <div
        className=" md:!hidden  !flex flex-col h-screen  transition-all bg-white  overflow-y-auto scrollbar-thin scrollbar-thumb-purple-200 scrollbar-track-purple-50 w-[75vw]"
        onClick={() => setViewNotifications(!viewNotifications)}
      >
        <div className="flex items-center  border-b border-[ rgb(209 213 219)] px-2 py-4 mb-2  drop-shadow-sm">
          ...
          <h2 className="font-nunito tracking-wide text-lg font-semibold text-gray-700 flex-1 ml-3">
            Notifications
          </h2>
        </div>
        {user?.groupNotifications.length > 0 ||
        chats?.some((chat) => checkMessage(chat)) ? (
          giveCombinedNotifications(user, chats)?.map((notification, index) => (
            <>
              <div
                key={index}
                className="w-[90%] mx-auto flex items-center rounded-md px-2 py-3 my-1.5 hover:bg-gray-100 cursor-pointer transition-all hover:drop-shadow-sm "
                onClick={() => handleNotification(notification, chats)}
              >
                <FiberManualRecordIcon
                  color="secondary"
                ...
                />
                {notification?.chat?.isGroupChat ? (
                  <p className="font-nunito text-xs w-full max-w-[75%] ">
                    New message recieved in{" "}
                    <span className="capitalize font-bold text-purple-800">
                      {notification?.chat?.groupName}
                    </span>{" "}
                ...
                {notification?.chat?.isGroupChat ? (
                  <div className="w-7 h-7 rounded-full ml-5">
                    <Avatar
                      src={notification?.chat?.groupImage}
                      alt="avatar"
