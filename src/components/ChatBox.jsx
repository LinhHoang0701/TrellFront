  ...
  return (
    ...
    <>
      <>
        ...
        {activechat?.users?.find((auser) => auser._id === user?._id) ? (
          <div className="hidden w-full h-[11%] mx-auto  bg-white p-2 drop-shadow-md border-t-[1.5px]  border-purple-700  absolute bottom-0 z-30 md:flex items-center">
            <IconButton
              sx={{ marginRight: "12px" }}
            ...
            <input
              ...
              className="py-2 px-3 w-[80%]  outline-none border border-slate-300 flex-1 "
              onKeyDown={sendMessage}
            />
            <IconButton
              sx={{ marginLeft: "14px", marginRight: "5px" }}
              size="large"
              onClick={sendMessageOnClick}
              disabled={message?.length === 0}
            >
              <SendIcon
                color={message?.length === 0 ? "disabled" : "secondary"}
              />
            </IconButton>
          </div>
        ) : (
          <div className="w-full h-[11%] mx-auto  bg-yellow-200  p-2 drop-shadow-md absolute bottom-0 z-50 flex items-center ">
            <p className="hidden font-nunito !font-semibold text-center mx-auto !text-gray-900 md:!flex items-center tracking-wide">
              <ImSad2 fontSize="1.4rem" className="mr-3" /> You are no longer a
        ...
        {activechat?.users?.find((auser) => auser._id === user?._id) ? (
          <div
            className={`md:hidden w-full  ${
              increaseHeight ? "[13vh]" : " h-[10vh]"
            } mx-auto  bg-white p-2 drop-shadow-md border-t-[1.5px]  border-purple-700  absolute bottom-2 z-50 flex items-center`}
          >
            <input
              id="message"
              autoComplete="off"
              ...
              placeholder="Message.."
              onChange={typingHandler}
              className="py-2.5 ml-4 px-3 w-[80%] text-sm font-nunito  outline-none border border-slate-300 flex-1 "
              onKeyDown={sendMessage}
            />
            <IconButton
              sx={{ marginLeft: "14px", marginRight: "5px" }}
          ...
          <div className="w-full h-[11vh] mx-auto  bg-yellow-400 bg-opacity-50 p-2 drop-shadow-md absolute bottom-0 z-50 flex items-center ">
            <p className="flex font-nunito !font-semibold text-center mx-auto text-gray-600 md:!hidden items-center text-sm tracking-wide">
              <ImSad2 fontSize="1.2rem" className="mr-3" /> You are no longer a
              part of this conversation.
            </p>
          </div>
        )}
