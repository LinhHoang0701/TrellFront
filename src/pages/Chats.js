...
const Chats = () => {
  ...
  async function fetchAllMessages() {
    ...
    try {
      const response = await axios.get(
        `${fetchAllMessagesRoute}/${activeChat?._id}`,
        {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("recieve message", (message) => {
      ...
      if (
        activeChatCompare === null ||
        activeChatCompare === undefined ||
        !activeChatCompare ||
        activeChatCompare?._id !== message?.chat?._id
      ) {
      ...
      <div className="hidden h-screen w-full md:flex items-center">
        ...
        {activeChat === undefined ? (
          ...
          <div className="relative flex flex-col items-center h-full w-[65%] mx-auto overflow-hidden">
            ...
            <div
              className={`hidden md:!flex md:flex-col md:h-full md:w-full ${styles.background} md:overflow-y-hidden  `}
            ></div>
            <ChatBox
              token={token}
              chats={chats}
      ...
      <div className="flex h-screen w-full md:!hidden ">
        {!singlechatMobileView ? (
          ...
          <div className="w-full h-screen flex md:!hidden flex-col">
            ...
            />
            <div
              className={`flex md:hidden flex-col h-[82vh] w-full  ${styles.background_mobile} overflow-y-hidden `}
            ></div>
            <ChatBox
              token={token}
              chats={chats}
