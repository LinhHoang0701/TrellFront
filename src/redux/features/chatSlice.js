...
import {
  callCreateSingleChatApi,
  callFetchChatsApi,
  callCreateGroupChatApi,
  callUpdateGroupImageApi,
  callRenameGroupApi,
  callRemoveSelfFromGroupApi,
  callDeleteGroupApi,
  callAddUsersToGroupApi,
  callAddUnreadMessagesApi,
  callRemoveUnreadMessagesApi,
} from "./chatThunkApiCalls";

export const accessSingleChat = createAsyncThunk(
  "chat/single/accessOrCreate",
  async (id, thunkAPI) => {
    try {
...
export const createGroupChat = createAsyncThunk(
  "chat/group/create",
  async ({ users, groupName }, thunkAPI) => {
    try {
      const token = JSON.parse(localStorage.getItem("token"));
      if (token) {
...
export const updateGroupImage = createAsyncThunk(
  "chat/group/image/update",
  async ({ chatId, groupImage }, thunkAPI) => {
    try {
      const token = JSON.parse(localStorage.getItem("token"));
      if (token) {
...
export const renameGroup = createAsyncThunk(
  "chat/group/rename",
  async ({ chatId, groupName }, thunkAPI) => {
    try {
      const token = JSON.parse(localStorage.getItem("token"));
      if (token) {
...
export const addUsersInGroup = createAsyncThunk(
  "chat/group/add",
  async ({ chatId, users }, thunkAPI) => {
    try {
      const token = JSON.parse(localStorage.getItem("token"));
      if (token) {
...
export const addToUnreadMessages = createAsyncThunk(
  "chat/unread/add",
  async ({ chat, message }, thunkAPI) => {
    try {
      const token = JSON.parse(localStorage.getItem("token"));
      if (token) {
...
export const removeUnread = createAsyncThunk(
  "chat/unread/remove",
...
export const chatSlice = createSlice({
  ...
  extraReducers: {
    ...
    [fetchAllChats.fulfilled]: (state, action) => {
      ...
      state.activeChat =
        state?.activeChat === undefined ? undefined : state?.activeChat;
    },
    ...
    [createGroupChat.fulfilled]: (state, action) => {
      state.isLoading = false;
      state.isSuccess = true;
      const existingGroupChat = state.chats.find(
        (chat) => chat._id === action.payload._id
      );
...
export const { reset, toggleChat, resetStatus, deleteFromList } =
  chatSlice.actions;
